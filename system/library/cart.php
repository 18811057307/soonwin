					...
					foreach ($options as $product_option_id => $value) {
						...
						if ($option_query->num_rows) {
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
                                ...
                                $option_value_query = $this->db->query("SELECT pov.option_value_id, od.name, pov.option_value, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_description od ON (od.option_id = pov.option_id) WHERE pov.product_option_value_id = '" . (int)$value . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
								...
								if ($option_value_query->num_rows) {
									...
									$option_data[] = array(
										...
										'value'                   => $option_value_query->row['option_value'],
