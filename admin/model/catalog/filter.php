...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET image = '" . $this->db->escape($filter['image']) . "',filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET image = '" . $this->db->escape($filter['image']) . "',filter_id = '" . (int)$filter['filter_id'] . "', filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET image = '" . $this->db->escape($filter['image']) . "',filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "'");
				}
    ...
    public function editChildFilter($parent_filter_id, $data) {
        $this->event->trigger('pre.admin.filter.edit', $data);

        $this->db->query("DELETE FROM " . DB_PREFIX . "filter_description WHERE filter_id IN (SELECT filter_id FROM" . DB_PREFIX . "filter WHERE $parent_filter_id = '" . (int)$parent_filter_id . "')");
        $this->db->query("DELETE FROM " . DB_PREFIX . "filter WHERE parent_filter_id = '" . (int)$parent_filter_id . "'");

        if (isset($data['filter'])) {
            foreach ($data['filter'] as $filter) {
                $pstmt = '';
                if ($filter['filter_id']) {
                    $pstmt = "INSERT INTO " . DB_PREFIX . "filter SET filter_group_id='0', image = '" . $this->db->escape($filter['image']) . "',filter_id = '" . (int)$filter['filter_id'] . "', parent_filter_id = '" . (int)$parent_filter_id . "', sort_order = '" . (int)$filter['sort_order'] . "'";
                } else {
                    $pstmt = "INSERT INTO " . DB_PREFIX . "filter SET filter_group_id='0', image = '" . $this->db->escape($filter['image']) . "',parent_filter_id = '" . (int)$parent_filter_id . "', sort_order = '" . (int)$filter['sort_order'] . "'";
                }
                $this->db->query($pstmt);
                ...
                foreach ($filter['filter_description'] as $language_id => $filter_description) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', filter_group_id = '0', name = '" . $this->db->escape($filter_description['name']) . "'");
                }
            }
        }

        $this->event->trigger('post.admin.filter.edit', $parent_filter_id);
    }
	...
	public function getFilterDescriptions($filter_group_id) {
		...
		$filter_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter WHERE filter_group_id = '" . (int)$filter_group_id . "' ORDER BY sort_order");
    ...
    public function getFilterDescription($filter_id) {
        ...
        $filter_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter WHERE filter_id = '" . (int)$filter_id . "'");
        ...
        foreach ($filter_query->rows as $filter) {
            ...
            $filter_data = array(
                'filter_id'          => $filter['filter_id'],
                'parent_filter_id'          => $filter['parent_filter_id'],
    ...
    public function getChildFilterDescriptions($parent_filter_id) {
        $filter_data = array();

        $filter_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter WHERE parent_filter_id = '" . (int)$parent_filter_id . "'  ORDER BY sort_order");
