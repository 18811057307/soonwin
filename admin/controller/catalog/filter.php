...
class ControllerCatalogFilter extends Controller {
    ...
    public function editChildFilter() {
        $this->load->language('catalog/filter');
        ...
        $returnParams = '';
        if (isset($this->request->get['parent_filter_id'])) {
            $data['parent_filter_description'] = $this->model_catalog_filter->getFilterDescription($this->request->get['parent_filter_id']);
            $returnParams .= '&filter_group_id=' . $data['parent_filter_description']['filter_group_id'];
        } else {
            $data['parent_filter_description'] = array();
        }

        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_catalog_filter->editChildFilter($this->request->get['parent_filter_id'], $this->request->post);
            ...
            $this->response->redirect($this->url->link('catalog/filter/edit', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
        ...
        $data['cancel'] = $this->url->link('catalog/filter/edit', 'token=' . $this->session->data['token'] . $returnParams  . $url, 'SSL');
        if (isset($this->request->get['parent_filter_id'])) {
            $data['action'] = $this->url->link('catalog/filter/editChildFilter', 'token=' . $this->session->data['token'] . '&parent_filter_id=' . $this->request->get['parent_filter_id'] . $url, 'SSL');
        }
        ...
        if (isset($this->request->post['filter'])) {
        ...
        } elseif (isset($this->request->get['parent_filter_id'])) {
            $this->load->model('tool/image');
            $data['filters'] = array();
            $filters = $this->model_catalog_filter->getChildFilterDescriptions($this->request->get['parent_filter_id']);
            foreach ($filters as $filter) {
                $data['filters'][] = array(
                    ...
                    'image'          => $filter['image'],
                    'thumb'      => $this->model_tool_image->resize( $filter['image'], 100, 100)
                );
            }
        } else {
            $data['filters'] = array();
        ...
        $this->response->setOutput($this->load->view('catalog/child_filter_form.tpl', $data));
    }
	...
	protected function getForm() {
		...
		if (isset($this->request->post['filter'])) {
            ...
            $this->load->model('tool/image');
            $data['filters'] = array();
			$filters = $this->model_catalog_filter->getFilterDescriptions($this->request->get['filter_group_id']);
            foreach ($filters as $filter) {
                $data['filters'][] = array(
                    'filter_id'          => $filter['filter_id'],
                    'filter_description' => $filter['filter_description'],
                    ...
                    'image'          => $filter['image'],
                    'thumb'      => $this->model_tool_image->resize( $filter['image'], 100, 100),
                    'edit'       => $this->url->link('catalog/filter/editChildFilter', 'token=' . $this->session->data['token'] . '&parent_filter_id=' . $filter['filter_id'] . $url, 'SSL'),
                );
            }
		} else {
			$data['filters'] = array();
