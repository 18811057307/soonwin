...
class ControllerSaleCustomer extends Controller {
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['credit'])) {
            ...
            $data['credit'] = $customer_info['credit'];
        } else {
            $data['credit'] = '';
        }

        if (isset($this->request->post['over_credit_status_id'])) {
            $data['over_credit_status_id'] = $this->request->post['over_credit_status_id'];
        } elseif (!empty($customer_info)) {
            $data['over_credit_status_id'] = $customer_info['over_credit_status_id'];
        } else {
            $data['over_credit_status_id'] = '';
        }
	...
	public function transaction() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/customer')) {
            ...
            if ((utf8_strlen($this->request->post['description']) < 2) || (utf8_strlen(trim($this->request->post['description'])) > 32)) {
                $this->error['description'] = $this->language->get('error_description');
            }

            if ((utf8_strlen($this->request->post['amount']) < 1) || (utf8_strlen(trim($this->request->post['amount'])) > 32)) {
