...
class ControllerCheckoutShippingMethod extends Controller {
	public function index() {
		...
		if (isset($this->session->data['shipping_address'])) {
            ...
            //设置默认的配送方式...TODO 根据商品或订单金额自动判定?
            if($method_data) {
                $defaultMethod = current($method_data);
                $defaultMethodQuote= current($defaultMethod['quote']);
                $shipping = explode('.', $defaultMethodQuote['code']);
                $this->session->data['shipping_method'] = $defaultMethod['quote'][$shipping[1]];
            }
        }
